...
- **College board (`/{slug}`)** = that college's "Suggestion box" with its **segments** as category chips (Mess 🍽 + sub, WiFi, Hostel, Library, Labs, Washroom, Sports, Parking, Other…), **Hot / Top / New** sorting, and the "File a complaint" button (→ `/{slug}/complaints/new`). College switcher dropdown to jump boards. Post/delete land back on `/{slug}` (not `/`).
