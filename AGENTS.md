...
- **Design v3 (Reddit-style, shipped)**: one **uniform #FF4500 orange** accent (no per-college themes — `components/CollegeTheme.tsx` deleted, `data-college` gone; DB `theme` column unused). Both **light** (bg #F7F7F8, white `.card`) and **dark** (bg #1A1A1B, surface #242426) modes. **Inter only** (Space Grotesk removed; `--font-display` = Inter). Design tokens live in `app/globals.css` `@theme` with unchanged names: `background/foreground/surface/surface-2/border/muted/accent/accent-strong/accent-soft/accent-ink/sage/sage-soft/plum/plum-soft` (sage/plum tokens still defined, currently unused). Default neutral Tailwind grays/zinc/emerald are used freely — do NOT re-map `--color-zinc-*` etc. Key utilities: `.card`, `.card-hover`, `.btn-primary` (solid orange pill), `.btn-accent` (soft orange), `.chip`/`.chip-active` (solid orange), `.section-label`, `.tap`. Global feed = two-column (`max-w-[1024px]`, `md:ml-60` sidebar) with right rail `hidden xl:flex w-72` (About / Top boards / About board / Segments cards); FeedCard = left vote rail (up arrow + "N votes", inline arrow+score below `sm`); sort tabs = Reddit border-b style with active orange underline. Nav sidebar: wordmark under a divider, and the active tab gets a **left orange indicator strip** (`absolute -left-2 h-[3px] rounded-full bg-accent`) — Reddit-style. Stagger entry animations were removed from feed/profile lists and **`.stagger` is deleted from `globals.css`** (keep lists static). Login page: Google button + a 3-row check-circle trust list below the card. `body` uses `text-rendering: optimizeLegibility` + antialiasing.
